...
const { handle } = require('../../../consumer/handlers/post-liked');
...
beforeEach(() => {
    jest.clearAllMocks();
    renderer.render.mockReturnValue('<html>post-liked</html>');
    sender.sendEmail.mockResolvedValue({ messageId: 'msg-003' });
    notificationRepository.updateLog.mockResolvedValue();
});

describe('post-liked handler', () => {
    it('calls renderer with the post-liked template and correct context', async () => {
        await handle(mockPayload, mockLog);

        expect(renderer.render).toHaveBeenCalledWith('post-liked', expect.objectContaining({
            recipientName: mockPayload.postAuthorName,
            likerName: mockPayload.likerName,
    ...
    it('calls sender with the correct recipient, subject, and html', async () => {
        ...
        expect(sender.sendEmail).toHaveBeenCalledWith(expect.objectContaining({
            ...
            subject: expect.stringContaining(mockPayload.likerName),
            html: '<html>post-liked</html>',
        }));
    });
